...
class Run():
    ...
    def get_dataloader(self,data_path):
        dataset=ECR_FND_Dataset(data_path,self.dataset)
        collate_fn=collate_fn_ECR_FND()
        dataloader = DataLoader(dataset, batch_size=self.batch_size, shuffle=True, num_workers=0, collate_fn=collate_fn)
        return dataloader

    def main(self):
        self.model = ECR_FND(self.dataset)
        ...
        if self.mode=='train':
            ...
            trainer=Trainer(model=self.model,device=self.device,lr=self.lr,dataloaders=dataloaders,epoches=self.epoches,model_name='ECR_FND',save_predict_result_path=save_predict_result_path,beta_c=self.alpha,beta_n=self.beta,early_stop=self.early_stop,save_param_path=self.path_ckp+self.dataset+"/")
            ...
            inferncer=Inferencer(model=self.model,device=self.device,model_name='ECR_FND',dataset=self.dataset,dataloader=dataloader,save_predict_result_path=save_predict_result_path)
