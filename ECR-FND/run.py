...
class Run():
    ...
    def main(self):
        ...
        if self.mode=='train':
            ...
            trainer=Trainer(model=self.model,device=self.device,lr=self.lr,dataloaders=dataloaders,epoches=self.epoches,model_name='ECR-FND',save_predict_result_path=save_predict_result_path,beta_c=self.alpha,beta_n=self.beta,early_stop=self.early_stop,save_param_path=self.path_ckp+self.dataset+"/")
            ...
            inferncer=Inferencer(model=self.model,device=self.device,model_name='ECR-FND',dataset=self.dataset,dataloader=dataloader,save_predict_result_path=save_predict_result_path)
