...
class ECR-FND_Dataset(Dataset):
    def __init__(self, vid_path,dataset):
        self.dataset=dataset

        if dataset=='fakesv': 
            self.data_all = pd.read_json('./datasets/fea/fakesv/metainfo.json',orient='records',dtype=False,lines=True)
            self.vid=[]
            ...
            self.descriptor_path = './datasets/fea/fakesv/descriptor_fea.pkl'
            with open(self.descriptor_path, 'rb') as f:
                self.descriptor_fea = torch.load(f)


            self.ocr_pattern_fea_path='./datasets/fea/fakesv/preprocess_ocr/sam'
            self.ocr_phrase_fea_path='./datasets/fea/fakesv/preprocess_ocr/ocr_phrase_fea.pkl'
            with open(self.ocr_phrase_fea_path, 'rb') as f:
                self.ocr_phrase = torch.load(f)

            self.text_semantic_fea_path='./datasets/fea/fakesv/preprocess_text/sem_text_fea.pkl'
            with open(self.text_semantic_fea_path, 'rb') as f:
                self.text_semantic_fea = torch.load(f)

            self.text_emo_fea_path='./datasets/fea/fakesv/preprocess_text/emo_text_fea.pkl'
            with open(self.text_emo_fea_path, 'rb') as f:
                self.audio_semantic_fea = torch.load(f)

            self.audio_semantic_path = './datasets/fea/fakesv/sem_audio_fea.pkl'
            with open(self.descriptor_path, 'rb') as f:
                self.descriptor_fea = torch.load(f)

            self.audio_fea_path='./datasets/fea/fakesv/preprocess_audio'
            self.visual_fea_path='./datasets/fea/fakesv/sliding_window_visual'

        elif dataset=='fakett':
            self.data_all=pd.read_json('./datasets/fea/fakett/metainfo.json',orient='records',lines=True,dtype={'video_id': str})
            ...
            self.descriptor_path = './datasets/fea/fakett/descriptor_fea.pkl'
            with open(self.descriptor_path, 'rb') as f:
                self.descriptor_fea = torch.load(f)



            self.ocr_pattern_fea_path='./datasets/fea/fakett/preprocess_ocr/sam'

            self.ocr_phrase_fea_path='./datasets/fea/fakett/preprocess_ocr/ocr_phrase_fea.pkl'
            with open(self.ocr_phrase_fea_path, 'rb') as f:
                self.ocr_phrase = torch.load(f)

            self.text_semantic_fea_path='./datasets/fea/fakett/preprocess_text/sem_text_fea.pkl'
            with open(self.text_semantic_fea_path, 'rb') as f:
                self.text_semantic_fea = torch.load(f)

            self.text_emo_fea_path='./datasets/fea/fakett/preprocess_text/emo_text_fea.pkl'
            with open(self.text_emo_fea_path, 'rb') as f:
            ...
            self.audio_fea_path='./datasets/fea/fakett/preprocess_audio'
            self.visual_fea_path='./datasets/fea/fakett/sliding_window_visual'
    ...
    def __getitem__(self, idx):
        ...
        all_audio_semantic_fea = self.audio_semantic_fea['last_hidden_state'][vid]
        ...
        ocr_pattern_fea = torch.load(open(ocr_pattern_fea_file_path, 'rb')).clone().detach()
        ...
        visual_segment = torch.tensor(torch.load(open(v_fea_path, 'rb')))

        all_descriptor_fea = self.descriptor_fea['last_hidden_state'][vid]
        ...
        return {
            ...
            'all_descriptor_fea': all_descriptor_fea,
            'all_audio_semantic_fea': all_audio_semantic_fea
...
def pad_segment(seg_lst,target_len):
    for sl_idx in range(len(seg_lst)):
        for s_idx in range(len(seg_lst[sl_idx])):
            ...
            seg_lst[sl_idx][s_idx] = seg_lst[sl_idx][s_idx].clone().detach()
...
def collate_fn_ECR-FND(batch):
    ...
    all_audio_semantic_fea = [item['all_audio_semantic_fea'] for item in batch]


    visual_segment = [item['visual_segment'] for item in batch]
    ...
    visual_segment, _ = pad_frame_sequence(num_visual_frames,visual_segment)
    ...
    visual_frames_fea,visual_frames_seg_indicator,sampled_seg=pad_frame_by_seg(num_visual_frames,visual_segment,visual_time_region)
    ...
    all_descriptor_fea = [item['all_descriptor_fea'] for item in batch]
