...
class Trainer():
    def __init__(self,model,device,lr,dataloaders,save_param_path,early_stop,epoches,model_name,save_predict_result_path,beta_c,beta_n,scheduler_option=False,save_threshold = 0.5, start_epoch = 0):
        self.model = model
        self.device = device
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.start_epoch+self.num_epochs):
            ...
            for batch in tqdm(self.dataloaders['train']):
                ...
                outputs,l_mr,l_info = self.model(**batch_data)
                                
                loss=self.CEloss(outputs, labels)+self.lambda_1*l_mr+self.lambda_2*l_info
            ...
            phase = 'val'
            wandb.log({
                f'Loss/{phase}': epoch_loss_val,
                f'Acc/{phase}': results_val['acc'],
                f'F1/{phase}': results_val['f1'],
                f'Auc/{phase}': results_val['auc'],
                f'Recall/{phase}': results_val['recall'],
                f'Precision/{phase}': results_val['precision']
            }, step=epoch)
